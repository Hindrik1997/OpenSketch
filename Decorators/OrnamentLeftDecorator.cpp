...
void OrnamentLeftDecorator::draw() const {
    ShapeDecorator::draw();
    Application::getInstance().renderText(m_text, m_shape->getPosition().x - m_shape->getSize().x / 2 - (m_shape->getSize().x * 1.5f), PAINT_WINDOW_SIZE_Y - m_shape->getPosition().y, 0.2, glm::vec3(0.0f,0.0f,0.0f));
}
